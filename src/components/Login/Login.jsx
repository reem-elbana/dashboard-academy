...
export default function Login() { 
  const [serverError, setServerError] = useState("");
  const [loading, setLoading] = useState(false);
  const [role, setRole] = useState("user"); // الافتراضي User
  const { saveToken } = useContext(AuthContext);
  const navigate = useNavigate();
  const { t } = useTranslation();
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      // تحديد API حسب الدور
      let apiUrl = "";
      if (role === "super_admin") apiUrl = "https://generous-optimism-production-4492.up.railway.app/api/auth/login";
      else if (role === "admin") apiUrl = "https://generous-optimism-production-4492.up.railway.app/api/auth/login";
      else apiUrl = "https://generous-optimism-production-4492.up.railway.app/api/auth/login"; // User
      ...
      if (normalizedRole === "super_admin") navigate("/super-admin/dashboard");
...
<div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gray-950">
  ...
  <div className="absolute inset-0 pointer-events-none">
    {["⚽","🏀","🎾","🏈","🏐","⚾"].map((icon,i)=>(
      <div key={i} className="absolute text-6xl opacity-10 animate-float-slow" style={{
        top: `${20 + i*12}%`,
        left: i%2===0 ? `${10 + i*15}%` : 'auto',
        right: i%2===1 ? `${10 + i*10}%` : 'auto',
        animationDelay: `${i*1.5}s`,
  ...
  <div className="relative z-10 w-full max-w-lg px-8">
    <div className="backdrop-blur-2xl bg-white/5 border border-green-500/30 rounded-3xl shadow-2xl overflow-hidden ring-1 ring-white/10 transform-gpu">
      ...
      {/* Role Selection */}
      <div className="flex justify-center gap-2 mb-4 me-5">
        <button onClick={()=>setRole("super_admin")} className={`px-4 py-2 rounded-xl ${role==="super_admin"?"bg-green-500":"text-green-600"}`}>Super Admin</button>
        <button onClick={()=>setRole("admin")} className={`px-4 py-2 rounded-xl ${role==="admin"?"bg-green-500":"text-green-600"}`}>Admin</button>
        <button onClick={()=>setRole("user")} className={`px-4 py-2 rounded-xl ${role==="user"?"bg-green-500":"text-green-600"}`}>User</button>
      </div>
